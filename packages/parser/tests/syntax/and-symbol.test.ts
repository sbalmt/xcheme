...
test('Consume the AND (symbol) expression in a SKIP pattern', () => {
  Assert.skip(`skip REF1 & REF2 & REF3;`, expression);
});

test('Consume the AND (symbol) expression in a TOKEN pattern', () => {
  Assert.token(`token <100> TOKEN as REF1 & REF2 & REF3;`, 'TOKEN', '100', expression);
});

test('Consume the AND (symbol) expression in a NODE pattern', () => {
