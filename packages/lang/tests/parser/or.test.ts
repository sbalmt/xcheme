...
test("Consume expected 'OR' rule", () => {
  const context = new Core.Context('test');
  const text = 'skip <REFERENCE1> or <REFERENCE2> or <REFERENCE3>;';
  ...
  expect(expr2.value).toBe(Parser.Nodes.Or);
  ...
  expect(expr1.value).toBe(Parser.Nodes.Or);
