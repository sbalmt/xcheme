...
test('Parse a MAP pattern in a dynamic TOKEN directive', () => {
  ...
  expect(tokenC.data.identity).toBe(0);
  Assert.tokens(context, [tokenA.data.identity, 101, tokenC.data.identity], 3);
});
...
test('Parse a MAP pattern in a NODE directive', () => {
  ...
  const ref2 = project.symbols.get('@REF1')!; // 'b'
  const ref4 = project.symbols.get('@REF3')!; // 'c'
  ...
  expect(nodeC.data.identity).toBe(2);
  Assert.nodes(context, [nodeA.data.identity, 201, nodeC.data.identity], 3);
});
