 ...
 */
const pattern = new MapFlowPattern(new Route(new RangeUnitPattern('0', '1'), '0', 'b'), new Route(new ChooseFlowPattern(new RangeUnitPattern('0', '9'), new RangeUnitPattern('a', 'f')), '0', 'x'));

test('Consume success', () => {
  const context = new Context('test');
  const source = new TextSource('0b10xf', context);
