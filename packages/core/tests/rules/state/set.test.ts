import { Context, TextSource, SetStatePattern, ExpectUnitPattern } from '../../../src/index';

/**
 * It can consume a sequence of characters 'a', 'b' and 'c', and emits a new error into the error list.
 */
const pattern = new SetStatePattern(0xabc, new ExpectUnitPattern('a', 'b', 'c'));
...
test('Consume success', () => {
  ...
  expect(source.output.state).toBe(0xabc);
});
